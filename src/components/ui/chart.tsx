...
type ChartTooltipContentProps = React.HTMLAttributes<HTMLDivElement> & {
  ...
  nameKey?: string
  labelKey?: string
  labelFormatter?: (value: unknown, payload?: ChartPoint[]) => React.ReactNode
  formatter?: (
    value: unknown,
    name: unknown,
    item: ChartPoint,
    index: number,
    payload: ChartPoint["payload"]
  ) => React.ReactNode
    ...
    return (
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item: ChartPoint, index: number) => {
            const key = `${nameKey || item.name || item.dataKey || "value"}`
            const itemConfig = getPayloadConfigFromPayload(config, item as unknown as Record<string, unknown>, key)
            const indicatorColor = color || (item.payload as Record<string, unknown> | undefined)?.fill || item.color
